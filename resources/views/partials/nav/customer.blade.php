...
$navbarClass = 'navbar navbar-dark bg-primary fixed-top'; // default
...
$wishlistCount = session('wishlist.count') ?? (is_array(session('wishlist.items')) ? count(session('wishlist.items')) : 0);
...
<nav id="mainNavbar" class="{{ $navbarClass }}">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Attach to ALL dropdowns inside navbar
    ...
    dropdowns.forEach(dd => {
        ...
        dd.addEventListener("hidden.bs.dropdown", function () {
            // Only reset if no dropdown is still open
            if (!navbar.querySelector(".dropdown.show")) {
